...
public class BlockingArrayQueue<E> extends AbstractList<E> implements BlockingQueue<E>
{
    ...
    @Override
    public E poll(){
        if (_size.get() == 0) //队列为空则返回null
            return null;

        E e = null;

        //poll从头部head取出数据，仅仅需要对头部head加锁，这样其他线程可以并发对尾部tail操作
        _headLock.lock(); // Size cannot shrink
        try{
            if (_size.get() > 0){
            	//根据HEAD_OFFSET得出可对头部进行poll操作的数据索引
                final int head = _indexes[HEAD_OFFSET];
                e = (E)_elements[head];
                _elements[head] = null;
                _indexes[HEAD_OFFSET] = (head + 1) % _elements.length;
                //poll取出数据后队列还有数据则发信号通知其他线程队列不为空
    ...
    public boolean offer(E e){
        Objects.requireNonNull(e); //对添加的e做null检查
        boolean notEmpty = false;
        
        //offer把数据添加至尾部tail，仅仅需要对尾部tail加锁，这样不影响其他线程并发的对头部head操作
        _tailLock.lock();
        try{
            int size = _size.get();
            if (size >= _maxCapacity) return false;//超过最大容量则offer失败

            //如果容量满了则进行扩容，扩容需要把整个队列加锁，所以此处还需要对头部head加锁
            if (size == _elements.length){
                _headLock.lock();
                try {
                    if (!grow()) return false;
                }finally{ _headLock.unlock(); }
            }

            //根据TAIL_OFFSET得出可对尾部进行offer操作的数据索引
            int tail = _indexes[TAIL_OFFSET];
            _elements[tail] = e;
            _indexes[TAIL_OFFSET] = (tail + 1) % _elements.length;
            
            //记录下offer添加数据前是否队列为空，notEmpty变量用于后续通知
            notEmpty = _size.getAndIncrement() == 0;
        }finally{ _tailLock.unlock(); }

        //offer添加数据前为空的话，则offer添加数据后非空，通知知其他线程队列非空
        if (notEmpty){ 
            _headLock.lock();
            try{
                _notEmpty.signal();
